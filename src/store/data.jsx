import React from 'react';

export const CodingTrack = ['Front End Wed Development (HTML, CSS, JavaScript)','Front End Wed Development (React Js)', 'Back End Web Development (Node Js)', 'Python + Django', 'Quality Assurance', 'Mobile Development (Flutter)'];
export const CodingTrackIcon = ['https://firebasestorage.googleapis.com/v0/b/tecthathonlandingpage.appspot.com/o/Code.png?alt=media&token=7be6b355-6c20-4e89-b311-c001cae5efa4'];
export const NonCodingTrack = ['Graphics design', 'UI/UX', 'Technical writing', 'Data Analytics', 'Microsoft Office', 'WordPress','Video Animation','3D Animation (Blender)', 'Product Management', 'Digtal Marketing and Content Creation'];
export const NonCodingTrackIcon = ['https://firebasestorage.googleapis.com/v0/b/tecthathonlandingpage.appspot.com/o/PenNib.png?alt=media&token=7f4459aa-9adf-41ce-8689-4c8d1f51f41a'];

export const Events = [
    {
        eventTitle: 'Future events',
        text:'Techathon 2.0 is expected to kickstart in the nearest future with lots of opportunities for you to explore, network and learn.'

    }, 
    {
        eventTitle: 'Community Hangout',
        text:'Techathon community hangout is an avenue for mentors and mentees to meet, relax, catch-up and connect physically. The hangout is free for everyone.'

    }, 
    {
        eventTitle: 'Campus Ambassador',
        text:'Do you want to make an impact in your school? become our campus ambassador in your school and preach our gospel to your follow students. No location or age restrictions.'

    }, 
];

export const AboutContent = [

    {
        title:"Welcome to Techathon community",
        image:"https://firebasestorage.googleapis.com/v0/b/tecthathonlandingpage.appspot.com/o/Image.png?alt=media&token=73b5985d-a122-4265-8ee4-19d1320f6ef9",
        desktopImage:"https://firebasestorage.googleapis.com/v0/b/tecthathonlandingpage.appspot.com/o/community.png?alt=media&token=0cccd4aa-fc58-4104-a407-c4ad22ddfae2",
        textLine1:"Techathon is a mentorship and open source community that is on a mission to help those who are transitioning into tech with the right resources they need to make their journey smooth, incusive and fun",  
        textLine2:"At Techathon, we have the right structure and environment to ensure you have a solid base in your career path.",
        textLine3:"At the community you get to  meet like minds to collaborate with and amazing professional mentors to guide you through every step of the learning process till you land your dream job. Our programs are virtual, beginner friendly and a lifetime access to the learning resources",
        textLine4:"Are you a seasoned tech individual interested in joining the community as a mentor? ",
    },
   
];

export const Logo = [
    {
        officalLogo:'https://firebasestorage.googleapis.com/v0/b/tecthathonlandingpage.appspot.com/o/Logo%202.png?alt=media&token=f442de33-6d8c-42a5-bf4d-d892fcc69154'
    }
]

export const Socials = [

    {
        icon:'https://firebasestorage.googleapis.com/v0/b/tecthathonlandingpage.appspot.com/o/tt.png?alt=media&token=506cffe7-5c75-4de6-b818-3331219b8da0'
    },
    {
        twicon:'https://firebasestorage.googleapis.com/v0/b/tecthathonlandingpage.appspot.com/o/tig.png?alt=media&token=3f35ddfc-f154-4ec7-82ee-bc41de2570f8'
    },
    {
        liicon:'https://firebasestorage.googleapis.com/v0/b/tecthathonlandingpage.appspot.com/o/linkedin.png?alt=media&token=dd6cf603-7359-4f2d-bbec-a95798b83087'
    },
    {
        fbicon:'https://firebasestorage.googleapis.com/v0/b/tecthathonlandingpage.appspot.com/o/facebook.png?alt=media&token=ca9c114e-ec59-4f0b-9ee6-ff0facf697a3'
    }
];

export const FooterNavigate  = [

    {
        navigate:"Home"
    },
    {
        comNavigate:"Components"
    },
    {
        evNavigate:"Events"
    },
    {
        atbNavigate:"About us"
    },
    {
        partNavigate:"Partners"
    },


];
export const FooterLinks  = [

    {
        foorterlinks:"Join waitlist"
    },
    {
        Codefoorterlinks:"Code of conduct"
    },
    {
        Mentorfoorterlinks:"Become a mentor"
    },
 


];

// Met our mentors array of objects
export const Mentors  =   [

    {
        image:'https://firebasestorage.googleapis.com/v0/b/tecthathonlandingpage.appspot.com/o/assets%2Fbg%2Fmentors%2Fphoto1659188210.jpeg?alt=media&token=3f410364-83cf-471a-9af0-a8dec4b1fab9',
        track:'React JS',
        name:'James Chigbo',
        professionandlocation:'Frontend Engr., Lagos Nigeria',
      
            twitter:'https://github.com/techathonian/landing-page',
            instagram:'https://github.com/techathonian/landing-page',
            linkein:'https://github.com/techathonian/landing-page'
        
    },
    {
        image:'https://firebasestorage.googleapis.com/v0/b/tecthathonlandingpage.appspot.com/o/assets%2Fbg%2Fmentors%2FDSC_0886.JPG?alt=media&token=d5a16b4d-aa0b-429a-9033-7c7e512b268c',
        track:'User Interface',
        name:'Sharach Lam',
        professionandlocation:'Product design, Lagos Nigeria'
    },
    {
        image:'https://firebasestorage.googleapis.com/v0/b/tecthathonlandingpage.appspot.com/o/assets%2Fbg%2Fmentors%2Fphoto1659188210.jpeg?alt=media&token=3f410364-83cf-471a-9af0-a8dec4b1fab9',
        track:'User Interface',
        name:'Sharach Lam',
        professionandlocation:'Product design, Lagos Nigeria'
    },
    {
        image:'https://firebasestorage.googleapis.com/v0/b/tecthathonlandingpage.appspot.com/o/assets%2Fbg%2Fmentors%2FAbass.jpeg?alt=media&token=04f330af-9b71-41d9-8977-89c35b7aa1c3',
        track:'Community manager',
        name:'Aisha Abass',
        professionandlocation:'Social Media Manager, Abuja Nigeria'
    },
    {
        image:'https://firebasestorage.googleapis.com/v0/b/tecthathonlandingpage.appspot.com/o/assets%2Fbg%2Fmentors%2FAbass.jpeg?alt=media&token=04f330af-9b71-41d9-8977-89c35b7aa1c3',
        track:'UI/UX',
        name:'James Aransiola',
        professionandlocation:'Product designer, Ondo Nigeria'
    },
    {
        image:'https://firebasestorage.googleapis.com/v0/b/tecthathonlandingpage.appspot.com/o/assets%2Fbg%2Fmentors%2FAbass.jpeg?alt=media&token=04f330af-9b71-41d9-8977-89c35b7aa1c3',
        track:'Community Manager',
        name:'Aderibigbe Adekorede',
        professionandlocation:'Front End Developer, Ondo Nigeria'
    },

];

// export const MentorsSocials = [

//     {
//         twitter:'',
//         instagram:'',
//         linkein:''
//     }
// ]
// read ipo fillings from cover to cover, build a website from scratch which include my analysis of the business and four ideas for  new features.
